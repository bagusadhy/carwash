...
@section('content')
        ...
        <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
            <div id="Service" class="flex flex-col gap-2">
                <div class="flex items-center w-full gap-[10px] bg-white justify-between">
                    <div class="flex items-center gap-[10px]">
                        ...
                        <div class="flex flex-col h-fit">
                            <div class="flex items-center gap-1">
                                <p class="font-semibold w-fit">{{ $booking->trx_id }}</p>
                    ...
                    <p class="rounded-full p-[6px_10px] {{ $booking->is_paid ? 'bg-[#41BE64]' : 'bg-[#FFCE51]' }} w-fit font-bold text-xs leading-[18px]">{{ $booking->is_paid ? 'PAID' : 'PENDING' }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
                ...
                <div class="flex items-center w-full gap-[10px] bg-white">
                    ...
                    <div class="flex flex-col">
                        ...
                        <div class="flex items-center gap-[2px]">
                            <p class="text-sm leading-[21px] text-[#909DBF]">{{ $booking->carStore->address }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="Time-details" class="flex flex-col gap-[10px]">
                <div class="flex items-center justify-between">
                    ...
                    <p class="font-semibold">{{ date('H:i', strtotime($booking->time_at)) }} WIB</p>
                </div>
                <div class="flex items-center justify-between">
                    <p class="text-sm leading-[21px]">Date At</p>
                    <p class="font-semibold">{{ date('d M Y', strtotime($booking->started_at)) }}</p>
                </div>
            </div>
            <hr class="border-[#E9E8ED]">
            <div id="Price-details" class="flex flex-col gap-[10px]">
                <div class="flex items-center justify-between">
                    ...
                    <p class="font-semibold">Rp {{ number_format($booking->carService->price, 0, ',', '.') }}</p>
                </div>
                <div class="flex items-center justify-between">
    ...
    <div class="px-8 mt-[30px] flex">
        <a href="tel:{{ $booking->carStore->phone_number }}" class="w-full rounded-full p-[12px_20px] bg-[#FF8E62] font-bold text-white text-center">Call
            Customer Service</a>
    </div>
@endsection
